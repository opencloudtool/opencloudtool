...
impl OrchestratorWithGraph {
    ...
    pub async fn deploy(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let spec_graph = graph::get_spec_graph(number_of_instances, &Self::INSTANCE_TYPE);

        let (resource_graph, vms) = graph::deploy_spec_graph(&spec_graph).await;
        let state = graph::State::from_graph(&resource_graph);
        let _ = infra_state_backend.save(&state).await?;
    ...
    pub async fn destroy(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        graph::destroy_graph(&resource_graph).await;
