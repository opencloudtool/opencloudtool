...
impl VMManager {
    fn create(&self) {
        println!("Create {}", self.resource.name);
    }
}
...
/// TODO: Test different types of resources in Node
#[derive(Default)]
enum Node {
...
fn get_graph(number_of_instances: i32) -> Graph<Node, String> {
    ...
    let vpc = deps.add_node(Node::Resource(ResourceType::VPC(VPC {
        name: String::from("VPC"),
        cidr: String::from("0000"),
    })));
    let vm = deps.add_node(Node::Resource(ResourceType::VM(VM {
        instance_type: String::from("t1"),
...
mod tests {
    ...
    fn test_get_graph() {
        ...
        let sorted = iterate_sequentially(&deps);
