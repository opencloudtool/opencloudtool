...
pub async fn deploy_spec_graph(graph: &Graph<SpecNode, String>) -> (Graph<Node, String>, Vec<Vm>) {
...
pub async fn destroy_graph(graph: &Graph<Node, String>) {
    // Load AWS configuration
    let region_provider = aws_sdk_ec2::config::Region::new("us-west-2");
    let config = aws_config::defaults(aws_config::BehaviorVersion::latest())
        .credentials_provider(
            ...
            match elem {
                ...
                Node::Resource(resource_type) => match resource_type {
                    ResourceType::Vpc(resource) => {
                        ...
                        let _ = manager.destroy(&resource).await.unwrap();
...
mod tests {
    ...
    async fn test_get_graph() {
        let deps = get_spec_graph();

        // log::info!("{}", Dot::new(&deps));
        //
        // deploy_graph(&deps).await;
    }
}
