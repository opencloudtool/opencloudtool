...
test('can add and edit multiple services', async ({ page }) => {
  ...
  await expect(servicesGrid.locator('h4', { hasText: 'frontend-app' })).toBeVisible();
  await expect(servicesGrid.locator('.bg-surface').filter({ hasText: 'frontend-app' }).getByText('my-repo/frontend:v1')).toBeVisible();
  await expect(servicesGrid.getByText('500m')).toBeVisible();

  await expect(servicesGrid.locator('h4', { hasText: 'backend-api' })).toBeVisible();
  await expect(servicesGrid.locator('.bg-surface').filter({ hasText: 'backend-api' }).getByText('my-repo/backend:latest')).toBeVisible();
