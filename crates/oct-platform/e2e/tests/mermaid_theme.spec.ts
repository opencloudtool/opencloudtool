...
test.describe('Mermaid Theme Responsiveness', () => {
    test('should re-render Mermaid diagram when theme is toggled', async ({ page, server }) => {
        // 1. Go to state page
        await page.goto(`${server.url}/projects/default/state`);

        // 2. Wait for mermaid to render
        const mermaidContainer = page.locator('.mermaid');
        await expect(mermaidContainer).toHaveAttribute('data-processed', 'true');

        // Capture initial SVG ID or content to compare later
        const initialSvgId = await mermaidContainer.locator('svg').getAttribute('id');
        ...
        const newSvgId = await mermaidContainer.locator('svg').getAttribute('id');
